...
def save_uope56_format_dynosam(root_path, sequence_name, ground_truth_obj_folder):
    ...
    # until maintainers change their file format
    object_pose_ground_truth_folder = os.path.join(root_path, "UOPE56_obj_pose")

    if not os.path.exists(ground_truth_file_path):
        print(f"No ground truth folder at {ground_truth_file_path}")
        return

    if not os.path.exists(object_pose_ground_truth_folder):
        print(f"No ground truth folder at {object_pose_ground_truth_folder}")
        return

    from pathlib import Path
    def find_sequence_dirs(base_dir, sequence_name):
        base = Path(base_dir)
        return [p for p in base.glob(f"*/{sequence_name}") if p.is_dir()]

    found_dirs = find_sequence_dirs(root_path, sequence_name)

    if(len(found_dirs) != 1):
        print(f"Could not find sequence folder {sequence_name}")

    sequence_folder = found_dirs[0]
    print(f"Found sequence folder at {sequence_folder}")

    # load camera trajectory in world
    with h5py.File(ground_truth_file_path, 'r') as f:
        timestamps = np.asarray(f["frame_tstamps"][:])
        T_w_g = np.asarray(f["gripper_to_world"][:])
        T_c_g = np.asarray(f["gripper_to_cam_constant"][:])
    T_g_c = invert_transform(T_c_g)
    # ground truth camera in world
    ...
    # ground truth object in world
    T_w_o = np.array([T_w_c[i] @ T_o_c[i] for i in range(T_o_c.shape[0])])

    assert(frame_id.shape[0] == timestamps.shape[0])
    assert(T_w_c.shape[0] == timestamps.shape[0])
    assert(T_w_o.shape[0] == timestamps.shape[0])

    output_dir = Path(sequence_folder)
    # now write ground truth camera pose and object pose to file
    # in csv format with timestamp, tx, ty, tz, qx, qy,qz, qw
    camera_csv = output_dir / "camera_poses.csv"
    object_csv = output_dir / "object_poses.csv"

    write_pose_csv_gtsam(camera_csv, timestamps, T_w_c)
    write_pose_csv_gtsam(object_csv, timestamps, T_w_o)

    print(f"Saved camera poses to: {camera_csv}")
    print(f"Saved object poses to: {object_csv}")
...
if __name__ == '__main__':
    ...
    # sequence_folder = "/root/data/dynoepts/UOPE56/others_50-55-004"
    sequence_name = "others_29"
